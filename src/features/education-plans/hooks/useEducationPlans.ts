...
import { EducationPlanRepository, type CreatePlanData, type UpdatePlanData, type PlanFilters } from '../../../lib/repositories/educationPlans'
 ...
 */
export function useEducationPlans(filters: PlanFilters = {}) {
 ...
 */
export function useEducationPlan(id: string | undefined) {
  const supabase = useSupabase()
  const repository = new EducationPlanRepository(supabase)
...
export function useDuplicateEducationPlan() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      id: string;
      overrides?: Partial<CreatePlanData>
    }) => {
      if (!session?.user?.id) {
...
export function useCanEditPlan(planId: string | undefined) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      if (!planId || !session?.user?.id) return false
 ...
 */
export function useEducationPlanStats(planId: string | undefined) {
  const supabase = useSupabase()
  const repository = new EducationPlanRepository(supabase)
