...
import { PlanEnrollmentRepository } from '../../../lib/repositories/planEnrollment';
/**
 * Hook to get user's enrollment status for a specific plan
 */
export function usePlanEnrollment(planId, userId) {
 ...
 */
export function usePlanEnrollments(planId) {
    const supabase = useSupabase();
    const repository = new PlanEnrollmentRepository(supabase);
    return useQuery({
...
export function useStartPlan() {
    ...
    const repository = new PlanEnrollmentRepository(supabase);
    return useMutation({
        mutationFn: ({ planId, firstTopicId }) => {
            if (!session?.user?.id) {
 ...
 */
export function useTopicProgress(topicId, userId) {
    const { session } = useSession();
    const currentUserId = userId || session?.user?.id;
    const supabase = useSupabase();
    const repository = new PlanEnrollmentRepository(supabase);
 ...
 */
export function useUserPlanTopicProgress(planId, userId) {
    const { session } = useSession();
    const currentUserId = userId || session?.user?.id;
    const supabase = useSupabase();
    const repository = new PlanEnrollmentRepository(supabase);
 ...
 */
export function usePlanEnrollmentStats(planId) {
    const supabase = useSupabase();
    const repository = new PlanEnrollmentRepository(supabase);
    return useQuery({
